...
- Sent only when the tool supports streaming (e.g. `execute_command`,
  `search_content`); zero or more frames, coalesced at ~100ms. **Never
  authoritative**: adapters may drop or ignore it, and the final `UF` frame
  always overwrites any preview content.
...
| `theme_list` | TUI only (not sent in NoTheme modes: plainio/terseio/rawio). `themes` (array of `{name:string, theme:{primary, dim, muted, warning, error, selection, added, removed, tool, fold_arrow, unfold_arrow: string}}`) — `fold_arrow`/`unfold_arrow` must fit one display cell; anything wider is replaced by the TUI's built-in default (see `docs/configuration.md`) | `SM-theme-list.bin` |
...
SM-theme-list.bin              {"type":"theme_list","data":{"themes":[{"name":"theme-dark","theme":{"primary":"#89d4fa","dim":"#313244","muted":"#6c7086","warning":"#f77923","error":"#f38ba8","selection":"#fab387","added":"#a6e3a1","removed":"#f38ba8","tool":"#f9e2af","fold_arrow":"▶","unfold_arrow":"▼"}},{"name":"theme-light","theme":{"primary":"#1e66f5","dim":"#ccd0da","muted":"#9ca0b0","warning":"#df8e1d","error":"#d20f39","selection":"#fe640b","added":"#40a02b","removed":"#d20f39","tool":"#df8e1d","fold_arrow":"▶","unfold_arrow":"▼"}}]}}
...
- `{{redirect_uri}}` → the adapter's local callback URL (e.g. `http://127.0.0.1:PORT/callback`)
- `{{state}}` → a random CSRF token generated by the adapter

The terminal adapter implements this via `startMCPAuthFlow`
