...
- **`Ctrl-C`** (SIGINT): cancels the current task through the session
  (same effect as typing `:cancel`) — tool processes are aborted cleanly
  and the session **continues**, so you can keep typing prompts. When no
  task is running, it reports `[error: nothing to cancel]`. The process
  exits only via `:quit`/`:q` or Ctrl-D; it never exits via Ctrl-C.
