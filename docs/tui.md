...
An expanded window's line is `- LABEL`, the arrival timestamp right-aligned to the window edge — and, while that line is pinned to the screen top, the count of the window's own lines hidden above it — and then the content. There is **no rule above and none below**: a window is opened by its own line, and closed by the next window's own line (the prompt box closes the last one). A *floating* surface — an overlay, the prompt input — still brackets itself with top and bottom rules, because nothing follows it.
