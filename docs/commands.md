...
|---------|-----|----------|----------|--------|
| `:quit` / `:q` | Shows confirmation dialog, then sends `quit` and exits when the session ends | Sends `quit`, then exits (code 0) | Sends `quit`, then exits (code 0) | Not interpreted — raw CI/CO pass-through |
