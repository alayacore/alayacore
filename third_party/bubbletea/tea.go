...
type View struct {
	...
	// Raw bypasses the cell-buffer renderer: the view content is written
	// VERBATIM to the terminal (after a full clear and cursor home) and
	// the terminal is left to soft-wrap it. This is required for soft-wrap
	// TUIs (REFACTOR.md) — the cell renderer truncates lines wider than
	// the screen and re-materializes wrapped rows as hard rows, which
	// breaks both the display and copy fidelity. Raw views redraw the
	// full frame every frame; the content must be padded so terminal
	// soft-wrap rows land exactly on the intended visual rows.
	Raw bool
