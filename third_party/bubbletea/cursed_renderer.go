...
func (s *cursedRenderer) flush(closing bool) error {
	...
	if !s.starting && !closing && s.lastView != nil && viewEquals(s.lastView, &view) &&
		(view.Raw || frameArea == s.cellbuf.Bounds()) {
		// No changes, nothing to do. Raw views skip the cell-buffer
		// bookkeeping (their frame is the whole screen regardless).
		return nil
	}
	...
	if view.Raw {
		// Raw passthrough (soft-wrap) mode: clear the whole screen, home
		// the cursor, and write the content VERBATIM — the terminal
		// soft-wraps it, so window fragments (continuous text padded to
		// the terminal width) land exactly on the intended visual rows
		// and selections copy without fake newlines. The cell renderer
		// cannot do this: it truncates lines wider than the screen and
		// re-materializes wrapped rows as hard rows.
		_, _ = s.scr.WriteString(ansi.EraseDisplay(2))
		_, _ = s.scr.WriteString(ansi.CursorHomePosition)
		_, _ = s.scr.WriteString(view.Content)
		if cur := view.Cursor; cur != nil {
			// Absolute CUP: the renderer's internal position model is
			// stale after a raw write (it does not track terminal
			// soft-wrap), so relative movement could aim at the wrong
			// cell. The soft-wrapped grid matches the visual rows, so
			// the absolute position lands on the right cell.
			_, _ = s.scr.WriteString(ansi.CursorPosition(cur.X+1, cur.Y+1))
		}
	} else {
