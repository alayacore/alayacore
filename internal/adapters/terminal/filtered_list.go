...
// WithSize sizes the filtered list to the terminal: the overlay box spans
// the FULL terminal width by design (all overlay input boxes are as wide
// as the terminal), so the box width and the filter input follow the
// terminal width; only the height is clamped to keep the list bounded.
...
// FilteredListResult describes what happened during HandleKey.
type FilteredListResult struct {
	// Handled is true if the key was consumed.
	Handled bool
...
func (fl FilteredListCore) HandleKey(msg KeyMsg) (FilteredListCore, FilteredListResult) {
	...
	if fl.FilterInputFocused {
		return fl.handleFilterFocusedKey(msg, keyStr)
	}
...
func (fl FilteredListCore) handleFilterFocusedKey(msg KeyMsg, key string) (FilteredListCore, FilteredListResult) {
	...
	if key == keyCtrlC {
		...
		return fl, FilteredListResult{Handled: true, FilterChanged: true}
	}
