...
func TestHelpWindowWithSize(t *testing.T) {
	...
	// By design the overlay box spans the full terminal width — WithSize
	// sizes the box (and the filter input) to the terminal.
	if hw.Width != 80 {
		t.Errorf("Expected terminal width 80, got %d", hw.Width)
	}
